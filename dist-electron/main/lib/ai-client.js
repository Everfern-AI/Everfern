...
class AIClient {
    ...
    async chat(request) {
        ...
        if (this.config.provider === 'everfern') {
            ...
            const hasImages = request.messages.some(m => Array.isArray(m.content) && m.content.some(c => c.type === 'image_url'));
            if (hasImages) {
    ...
    async _openAISDKChat(req) {
        ...
        try {
            ...
            if (isStreaming) {
                ...
                const stream = await retryWithBackoff(() => this.openaiClient.chat.completions.create({
                    ...options,
                    stream: true
                }));
                let fullContent = '';
                ...
                // Non-streaming mode
                const response = await retryWithBackoff(() => this.openaiClient.chat.completions.create(options));
    ...
    async *_openAISDKStream(req) {
        ...
        try {
            ...
            const stream = await retryWithBackoff(() => this.openaiClient.chat.completions.create(options));
