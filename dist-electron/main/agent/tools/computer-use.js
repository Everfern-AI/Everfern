...
class ComputerUseAgent {
    ...
    constructor(client, tool, model, task, temperature = 0, maxTurns = 40, historyWindow = 12, toolCallId = '') {
        this.client = client;
        this.tool = tool;
        this.model = model;
        this.task = task;
     ...
     */
    logActionTiming(action, durationMs) {
    ...
    endActionTiming() {
        if (this.currentActionStartTime === 0)
            return 0;
...
function createComputerUseTool(originalClient, _platform = process.platform, _visionModel, _showuiUrl, _ollamaBaseUrl, _checkPermission, _requestPermission, vlm) {
    ...
    return {
        ...
        async execute(args, onUpdate, emitEvent, toolCallId) {
            ...
            const agent = new ComputerUseAgent(subAgentClient, tool, subAgentModel, task, 0, // temperature
            40, // maxTurns
            12, // historyWindow
            toolCallId || '');
            ...
            const progressEmitter = toolCallId && emitEvent
                ? new ProgressEventEmitter(toolCallId, customSender)
                : null;
            if (progressEmitter) {
            ...
            try {
                const { finalAnswer, lastScreenshot } = await agent.run((update) => {
                    onUpdate?.(update);
                    overlay.show(update);
                }, (event) => {
                    ...
                    if (event.type === 'screenshot' && event.screenshotPath) {
                        screenshotPaths.push(event.screenshotPath);
                    }
