...
import ToolDetailSidePanel from '../tools/ToolDetailSidePanel';
...
function TaskSection({
  ...
  // Calculate progress
  const completedPills = task.pills.filter((p: ToolPill) => p.status === 'completed').length;
  const failedPills = task.pills.filter((p: ToolPill) => p.status === 'failed').length;
...
export default function PillNarrativeTimeline({
  ...
  const displayTasks = useMemo(() => {
    ...
    return timeline.tasks.map((task: Task) => {
